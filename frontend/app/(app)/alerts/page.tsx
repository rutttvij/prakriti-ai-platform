...
export default function AlertsPage() {
  ...
  const monitoringQuery = useQuery({ queryKey: ["monitoring", "bundle", locationParams], queryFn: () => getMonitoringData(locationParams), refetchInterval: 120000 });
  ...
  const { statusMap } = useIssueStatusState("alerts");

  const alertsWithStatus = useMemo(
    () => withStatusOverrides(monitoringQuery.data?.alerts ?? [], statusMap),
    [monitoringQuery.data?.alerts, statusMap],
  );

  const typeOptions = useMemo(() => Array.from(new Set(alertsWithStatus.map((item) => item.type))), [alertsWithStatus]);
  const ownerOptions = useMemo(() => Array.from(new Set(alertsWithStatus.map((item) => item.assigned_owner_id).filter((item): item is string => Boolean(item)))), [alertsWithStatus]);

  const filtered = useMemo(
    () => applyMonitoringFilters(alertsWithStatus, {
      type,
      severity,
      status,
      city_id: cityId,
    ...
    [alertsWithStatus, type, severity, status, cityId, wardId, zoneId, assignedOwner, dateFrom, dateTo],
  );

  if (monitoringQuery.isLoading) {
    return <LoadingState title="Loading alerts" description="Collecting operational alert indicators." />;
  }

  if (monitoringQuery.isError) {
    return <ErrorState title="Unable to load alerts" description="Please retry monitoring query." onRetry={() => void monitoringQuery.refetch()} />;
  }

  return (
    <div className="space-y-6">
      <PageHeader title="Operational Alerts" description="Triage operational, compliance, and verification alerts." />
      ...
      <FilterBar onReset={() => {
        ...
        <FormSelectField label="Alert Type" value={type} onChange={setType} options={[{ label: "All", value: ALL }, ...typeOptions.map((item) => ({ label: item, value: item }))]} />
      ...
      <div className="grid gap-4">
        ...
        {!filtered.length ? <p className="text-sm text-slate-500">No alerts found for the selected filters.</p> : null}
      </div>
    </div>
  );
}
