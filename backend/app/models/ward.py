...
class Ward(UUIDPrimaryKeyMixin, TimestampMixin, AuditMixin, ActiveMixin, Base):
    ...
    worker_profiles = relationship("WorkerProfile", back_populates="ward")
    vehicles = relationship("Vehicle", back_populates="ward")
    shifts = relationship("Shift", back_populates="ward")
    routes = relationship("Route", back_populates="ward")
    pickup_tasks = relationship("PickupTask", back_populates="ward")
    processing_facilities = relationship("ProcessingFacility", back_populates="ward")
    collected_batches = relationship("CollectedBatch", back_populates="ward")
    carbon_projects = relationship("CarbonProject", back_populates="ward")
    carbon_ledger_entries = relationship("CarbonLedgerEntry", back_populates="ward")
    environmental_summaries = relationship("EnvironmentalSummary", back_populates="ward")
